...
class html{
 ...
 crearTemplateFlotaCamiones(aCamiones:any) {
    fs.writeFileSync('./src/compartido/html.html',cabecera)
    fs.appendFileSync('./src/compartido/html.html',separador)
    aCamiones.forEach((element: { patente: string, kilometraje: number, ultimoServiceAceite:number, ultimoServiceNeumatico:number, ultimoServiceFiltro:number }) => {pat=element.patente, km=element.kilometraje,aceite=element.ultimoServiceAceite,neumatico=element.ultimoServiceNeumatico,filtro=element.ultimoServiceFiltro
